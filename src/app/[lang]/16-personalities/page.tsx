...
export async function generateMetadata({ params }: IProps): Promise<Metadata> {
  ...
  return {
    title: title || t('site.personalities_page.seo.title'),
    description: description || t('site.personalities_page.seo.description'),
    keywords: keywords || t('site.personalities_page.seo.keywords'),
...
export default async function Personalities({ params: { lang } }: IProps) {
    ...
    <div className="page page-default page-problems">
      <Container size="sm">
        <Title size="sm" isCenter className="page-default-title" tag="h1">
          {t('site.personalities_page.title')}
        </Title>
        <Breadcrumbs
          items={[
            {
              label: t('site.breadcrumbs.personalities'),
...
const getPosts = async ({ lang }: { lang: LangType }) => {
  const data =  await serverFetch(`/page?template=${PAGE_TEMPLATES.PERSONALITIES}`, { lang });
