...
export async function generateMetadata({ params }: IProps): Promise<Metadata> {
  ...
  return {
    title: title || t('site.dorosly_temy_page.seo.title'),
    description: description || t('site.dorosly_temy_page.seo.description'),
    keywords: keywords || t('site.dorosly_temy_page.seo.keywords'),
...
export default async function DoroslyTemy({ params: { lang } }: IProps) {
    ...
    <div className="page page-default page-problems">
      <Container size="sm">
        <Title size="sm" isCenter className="page-default-title" tag="h1">
          {t('site.dorosly_temy_page.title')}
        </Title>
        <Breadcrumbs
          items={[
            {
              label: t('site.breadcrumbs.dorosly_temy'),
...
const getPosts = async ({ lang }: { lang: LangType }) => {
  const data =  await serverFetch(`/page?template=${PAGE_TEMPLATES.DOROSLY_TEMY}`, { lang });
