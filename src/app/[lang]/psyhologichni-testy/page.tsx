...
export async function generateMetadata({ params }: IProps): Promise<Metadata> {
  const slug = 'psyhologichni-testy';
  const { lang } = params;
  ...
  return {
    title: title || t('site.psyhologichni_testy.seo.title'),
    description: description || t('site.psyhologichni_testy.seo.description'),
    keywords: keywords || t('site.psyhologichni_testy.seo.keywords'),
...
export default async function PsyTests({ params: { lang } }: IProps) {
    ...
    <div className="page page-default page-problems">
      <Container size="sm">
        <Title size="sm" isCenter className="page-default-title" tag="h1">
          {t('site.psyhologichni_testy.title')}
        </Title>
        <Breadcrumbs
          items={[
            {
              label: t('site.breadcrumbs.psy_test'),
...
const getPosts = async ({ lang }: { lang: LangType }) => {
  return await serverFetch(`/page?template=${PAGE_TEMPLATES.PSY_TEST}`, { lang });
};